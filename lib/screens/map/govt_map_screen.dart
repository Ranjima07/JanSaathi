...
class _GovtMapScreenState extends State<GovtMapScreen> {
  ...
  // Default position (e.g., Kerala)
  LatLng _currentPosition = const LatLng(10.8505, 76.2711);
  bool _isLoading = true;

  // IMPORTANT: Secure your API key in production!
  ...
  Future<void> _determinePosition() async {
    ...
    if (permission == LocationPermission.deniedForever) {
      _showError("Permissions are permanently denied.");
      return;
    }
    ...
    // FIX 1: Manually move the camera to the new position
  ...
  Future<void> _fetchPlaces({String? type, String? keyword}) async {
    final url =
        "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${_currentPosition.latitude},${_currentPosition.longitude}&radius=5000&keyword=${keyword ?? ""}&type=${type ?? ""}&key=$googleApiKey";
              ...
              _markers.add(
                Marker(
                  ...
                  icon: BitmapDescriptor.defaultMarkerWithHue(
                    type == "hospital" ? BitmapDescriptor.hueRed : BitmapDescriptor.hueAzure
                  ),
